        ...
        <div x-show="modalOpen"
            ...
            <div @if ($closeOutside) @click.self="modalOpen=false" @endif class="relative flex min-h-full items-start justify-center p-4 sm:items-center">
                <div id="{{ $modalId }}" x-show="modalOpen" x-trap.inert.noscroll="modalOpen"
                    ...
                    class="relative flex max-h-[calc(100dvh-2rem)] w-full flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-modal dark:border-white/10 dark:bg-panel lg:w-auto lg:min-w-2xl lg:max-w-4xl">
                    <div class="flex items-center justify-between py-5 px-6 shrink-0 border-b border-neutral-200 dark:border-white/[0.06]">
                        <h3 class="text-lg font-semibold text-black dark:text-fg">{{ $title }}</h3>
